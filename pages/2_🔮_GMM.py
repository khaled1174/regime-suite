...
with st.sidebar:
    ...
    n_states = st.selectbox("GMM Components", [2, 3, 4], index=1)
    st.markdown("---")
    ...
    <b style='color:#a371f7'>Key:</b> Soft clustering with probability assignments.
    </div>
...
if not run_btn:
    ...
    <div style='text-align:center;padding:40px 0;color:#8b949e'>
      <p style='font-size:.95rem;margin:0'>Configure and click <b style="color:#3fb950">Run GMM Analysis</b></p>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
with st.spinner("Fitting Gaussian Mixture Model…"):
...
with st.expander("🔬 GMM Model Details"):
    ...
    st.dataframe(prob_df.style.format("{:.3f}"), width="stretch")

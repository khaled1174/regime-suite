...
with st.sidebar:
    ...
    n_states = st.selectbox("Clusters", [2, 3, 4, 5], index=1)
    st.markdown("---")
    ...
    <b style='color:#f0b429'>Best for:</b> Exploratory analysis and baseline.
    </div>
...
if not run_btn:
    ...
    <div style='text-align:center;padding:40px 0;color:#8b949e'>
      <p style='font-size:.95rem;margin:0'>Configure and click <b style="color:#3fb950">Run K-Means Analysis</b></p>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
with st.expander("🔬 K-Means Details"):
    ...
    st.dataframe(centers_df.style.format("{:.6f}"), width="stretch")
